...
@unittest.skipUnless(torch.cuda.device_count() > 0, "Skipping test: Requires GPUs")
...
class TestMLLMTRTModel(unittest.TestCase):
    @patch.dict(os.environ, {"ONELLM_VLM_ENABLE_TRT": "1"}, clear=True)
    def test_inference_trt(self):
        from lmdeploy import GenerationConfig, TurbomindEngineConfig, pipeline, VisionConfig
        self.session_len = 8192
        self.extra_args.update({"tp": 1})
