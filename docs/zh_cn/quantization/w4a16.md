...
- `--dataset-path XXX`  
  指定本地数据集的路径 **或** 从 AIS 加载指定的数据集，格式为 `dataset_id_commit_id`。

- `--prompt-file xxx`  
  指定本地 prompt 文件的名称，prompt 文件应包含 `prompt` 和对应的图片名称，支持解析 `jsonl`、`csv`、`json` 文件。文件路径为 `dataset-path/prompt-file`。

- `--image-folder xxx`  
  指定图片存储的文件夹，图片路径为 `dataset-path/image-folder/image-name`，默认文件夹名称为 `images`。

- `--ais-dataset True`  
  是否选择从 AIS 加载数据集，默认为 `False`。
...
#### 3. 从 AIS 加载数据集
AIS 数据集具体的上传过程请参考 [AIS Dataset SDK](https://doc.mlp.shopee.io/AIP/Modules/AIS%20Dataset%20&%20SDK/sdk)

```bash
--calib-dataset llvm --ais-dataset True --dataset-path datasetID_commitID
```

具体可以参考 [lmdeploy 多模态数据集做 AWQ 校准](https://docs.google.com/document/d/1E1Rzbos6_iZsEYPm0_FXND6XkR1TNnJ13Vgm4AMXfB8)
