...
The following parameters have been added to the original command:

- `--calib-dataset llvm`  
  Specifies using a multimodal format for calibration.

- `--dataset-path XXX`  
  Specifies the local dataset path **or** loads a dataset from AIS. When loading from AIS, the format should be `dataset_id_commit_id`.

- `--prompt-file xxx`  
  Specifies the name of the local prompt file. The prompt file should include `prompt` and the corresponding image name, supporting `jsonl`, `csv`, and `json` formats. The file path will be `dataset-path/prompt-file`.

- `--image-folder xxx`  
  Specifies the folder where images are stored. The image path will be `dataset-path/image-folder/image-name`. The default folder name is `images`.

- `--ais-dataset True`  
  Indicates whether to load the dataset from AIS. Defaults to `False`.

---

### Dataset Loading Methods

The extension provides the following three methods for loading datasets:

#### 1. Use the Default Multimodal Dataset WIT
...
```bash
--calib-dataset llvm --dataset-path XXX --prompt-file xxx --image-folder xxx
```

#### 3. Load a Dataset from AIS
For details on uploading AIS datasets, refer to the AIS Dataset SDK Documentation.

```bash
--calib-dataset llvm --ais-dataset True --dataset-path datasetID_commitID
```

For details, please refer to [lmdeploy 多模态数据集做 AWQ 校准](https://docs.google.com/document/d/1E1Rzbos6_iZsEYPm0_FXND6XkR1TNnJ13Vgm4AMXfB8)
