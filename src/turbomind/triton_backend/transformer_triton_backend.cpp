...
std::vector<std::vector<ft::NcclParam>>
AbstractTransformerModel::createNcclParams(const int node_id, const int device_id_start, const bool multi_node)
{
    ...
    if (tensor_para_size > 1 || pipeline_para_size > 1) {
        ...
        // TODO(Alan): for mpi multi gpu support
    ...
    // NOTE(Alan): 当前EP和TP在MoE层是互斥的，后续需要考虑是否进行独立处理
    if (tensor_para_size > 1) {
        ...
        for (int gid = device_id_start; gid < device_id_start + local_comm_size; gid++) {
            ...
            ft::ftNcclCommInitRank(tensor_para_params[gid - device_id_start],
                                   tensor_para_rank,
                                   tensor_para_size,
    ...
    return std::vector<std::vector<ft::NcclParam>>{tensor_para_params, pipeline_para_params};
}
