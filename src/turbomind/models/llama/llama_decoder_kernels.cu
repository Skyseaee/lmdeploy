...
struct res_norm_t {
    ...
    __device__ short4 quant(const uint4& u, float scale) const
    {
        short4 v;
        v.x = f.quant(u.x, scale);
        v.y = f.quant(u.y, scale);
        v.z = f.quant(u.z, scale);
        v.w = f.quant(u.w, scale);
        return v;
    }
};
...
#ifdef ENABLE_FP8
    __device__ short quant(const uint& x, float scale) const
    {
        auto a = reinterpret_cast<const half2&>(x);
        auto z0 = __nv_fp8_e4m3(float(a.x) * scale);
        auto z1 = __nv_fp8_e4m3(float(a.y) * scale);
        auto z = __nv_fp8_2_e4m3{z0, z1};
        return reinterpret_cast<short&>(z);
    }
#endif
};
...
struct res_norm_ops_t<float> {
    ...
    __device__ short quant(const uint& x, float scale) const
    {
        // TODO(Alan): not support fp8 with float forward
    }
};
...
struct res_norm_ops_t<__nv_bfloat16> {
    ...
    __device__ short quant(const uint& x, float scale) const
    {
        auto a = reinterpret_cast<const __nv_bfloat162&>(x);
        auto z0 = __nv_fp8_e4m3(float(a.x) * scale);
        auto z1 = __nv_fp8_e4m3(float(a.y) * scale);
        auto z = __nv_fp8_2_e4m3{z0, z1};
        return reinterpret_cast<short&>(z);
...
// r' = r + x
// t' = norm(r') * scales
// x' = __nv_fp8_e4m3(t' * qscale)
template<typename T>
__global__ void fusedAddBiasResidualNormAndQuant(T* __restrict__ r_data,
                                                 __nv_fp8_e4m3* __restrict__ o_q_data,
                                                 __nv_fp8_e4m3* __restrict__ moe_o_q_data,
                                                 T* __restrict__ o_data,
                                                 T* __restrict__ x_data,
                                                 const T* __restrict__ bias,
                                                 const T* __restrict__ scale,
                                                 float eps,
                                                 ...
                                                 int   n_dims)
{
    ...
    uint4* __restrict__ r_ptr        = reinterpret_cast<uint4*>(r_data + batch_idx * n_dims);
    uint4* __restrict__ x_ptr        = reinterpret_cast<uint4*>(x_data + batch_idx * n_dims);
    short4* __restrict__ o_q_ptr     = reinterpret_cast<short4*>(o_q_data + batch_idx * n_dims);
    short4* __restrict__ moe_o_q_ptr = moe_o_q_data ? reinterpret_cast<short4*>(moe_o_q_data + batch_idx * n_dims) : nullptr;
    uint4* __restrict__ o_ptr        = o_data ? reinterpret_cast<uint4*>(o_data + batch_idx * n_dims) : nullptr;
    const uint4* __restrict__ b_ptr  = reinterpret_cast<const uint4*>(bias);
    ...
    for (uint i = block.thread_rank(); i < n_dims / PACK_DIM; i += block.size()) {
        ...
        auto s   = s_ptr[i];
        auto o   = ops.normvec(r, s, s_inv_mean);
        if(o_ptr!=nullptr) o_ptr[i] = o;
        if(moe_o_q_ptr!=nullptr) moe_o_q_ptr[i] = ops.quant(o, moe_qscale);
        o_q_ptr[i] = ops.quant(o, qscale);
    }
}

template<typename T>
void invokeFusedAddBiasResidualRMSNormAndQuant(
    T* residual, __nv_fp8_e4m3* out_q, __nv_fp8_e4m3* moe_out_q, T* out, T* in, const T* bias, const T* scale, float eps, float qscale, float moeqscale, int batch_size, int n_dims, cudaStream_t stream)
{
    constexpr int PACK_DIM = sizeof(uint4) / sizeof(T);
    FT_CHECK(n_dims % PACK_DIM == 0);
    const int n_pack    = n_dims / PACK_DIM;
    ...
    fusedAddBiasResidualNormAndQuant<<<batch_size, n_threads, 0, stream>>>(
        residual, out_q, moe_out_q, out, in, bias, scale, eps, qscale, moeqscale, batch_size, n_dims);
}
#endif
...
template void 
invokeFusedAddBiasResidualRMSNormAndQuant(float*, __nv_fp8_e4m3*, __nv_fp8_e4m3*, float*, float*, const float*, const float*, float, float, float, int, int, cudaStream_t);
#endif
...
#ifdef ENABLE_FP8
template void 
invokeFusedAddBiasResidualRMSNormAndQuant(half*, __nv_fp8_e4m3*, __nv_fp8_e4m3*, half*, half*, const half*, const half*, float, float, float, int, int, cudaStream_t);
#endif
#ifdef ENABLE_BF16
...
#ifdef ENABLE_FP8
template void 
invokeFusedAddBiasResidualRMSNormAndQuant(__nv_bfloat16*, __nv_fp8_e4m3*, __nv_fp8_e4m3*, __nv_bfloat16*, __nv_bfloat16*, const __nv_bfloat16*, const __nv_bfloat16*, float, float, float, int, int, cudaStream_t);
#endif
#endif
}  // namespace turbomind
