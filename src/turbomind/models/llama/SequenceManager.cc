...
const Sequence* SequenceManager::Create(uint64_t id)
{
    ...
    if (it != sequences_.end()) {
        if (rank_ == 0) {
            TM_LOG_WARNING("[SeqMgr][Create] Removing conflicting ID %lld", id);
        }
        Erase(it);
    }
...
void SequenceManager::CachePrompt(const Sequences& sequences, int active_size)
{
    ...
    for (int i = 0; i < active_size; ++i) {
        ...
        std::tie(block_ids, block_unique_ids) = block_trie_->Insert(seq, seq.prompt);
        if (rank_ == 0) {
...
void SequenceManager::PrefixMatch(Sequences& sequences)
{
    ...
    for (int i = 0; i < sequences.size(); ++i) {
        ...
        std::tie(block_ids, unique_ids) = block_trie_->MatchPrefix(seq);
                                  ...
                                  AdjustInputCount             adjust) -> Outcome
{
    ...
    TM_LOG_DEBUG("active: %4d, cached: %4d, free: %4d",
