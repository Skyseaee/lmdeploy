...
void _runGemm(int const M, int const N, int const K, bool const transA, bool const transB,
    const turbomind::DataType type, CublasGemmWrapperPtr const& cublasWrapperPtr, void const* act, void const* weight,
...
LoraImpl::LoraImpl(int in_hidden_size, std::vector<int> out_hidden_sizes, int transA, int transB, int num_lora_modules,
    turbomind::DataType type, int max_low_rank, std::shared_ptr<CublasGemmWrapper> cublasWrapper)
...
{
    TM_LOG_ERROR("%s, Do not support Lora temporary!", __PRETTY_FUNCTION__);
//     TLLM_LOG_DEBUG("%s", __PRETTY_FUNCTION__);
...
size_t LoraImpl::getWorkspaceSize(
    int64_t const numTokens, int64_t const numReqs, turbomind::DataType const type) const noexcept
{
    TM_LOG_ERROR("%s, Do not support Lora temporary!", __PRETTY_FUNCTION__);
    return 0;
    // TLLM_LOG_DEBUG("%s", __PRETTY_FUNCTION__);
    ...
    void const* const* loraWeightsPtr, int weightIndex, void* const* outputs, void* workspace, cudaStream_t stream)
{
    ...
    //         if (N > 0)
    //         {
    //             FT_CHECK_WITH_INFO(N <= mMaxLowRank,
    ...
    //     {
    //         FT_CHECK_WITH_INFO(mTransA == false && mTransB == true,
...
int Lora_run(LoraImpl* impl, int64_t numTokens, int64_t numReqs, void const* input, int32_t const* loraRanks,
    void const* const* loraWeightsPtr, int weightIndex, void* const* outputs, void* workspace, cudaStream_t stream)
{
    FT_CHECK_WITH_INFO(0, "Attempt to run an empty LoraImpl");
    return 0;
    //FT_CHECK_WITH_INFO(impl != nullptr, "Attempt to run an empty LoraImpl");
