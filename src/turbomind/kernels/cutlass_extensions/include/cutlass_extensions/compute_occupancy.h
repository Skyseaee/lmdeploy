...
inline int compute_occupancy_for_kernel()
{
    ...
    if (smem_size > (48 << 10))
    {
        ...
        {
            turbomind::check_cuda_error(cudaFuncGetAttributes(&attr, cutlass::device_kernel<GemmKernel>));
        }
        else
        {
            turbomind::check_cuda_error(cudaFuncGetAttributes(&attr, cutlass::Kernel<GemmKernel>));
        }
        ...
        if constexpr (enable_cutlass_3x)
        {
            turbomind::check_cuda_error(cudaFuncSetAttribute(
