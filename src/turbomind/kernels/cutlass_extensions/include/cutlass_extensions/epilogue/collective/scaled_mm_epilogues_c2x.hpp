...
#include "cutlass_extensions/epilogue/collective/broadcast_load_epilogue_c2x.hpp"
...
struct ScaledEpilogueBase {
 protected:
  ...
  static auto args_from_tensor(const T* tensor, const int tensor_num) {
    ...
    //auto* data_ptr = static_cast<T*>(tensor);
  ...
  template <typename Descriptor, typename T>
  static auto args_from_tensor(std::optional<T*> const& tensor) {
    static_assert(std::is_same_v<Descriptor, RowOrZeroLoad<T>>);
    using Arguments = typename Descriptor::Arguments;
    //auto* data_ptr = tensor ? static_cast<T*>(tensor) : nullptr;
    T* data_ptr = tensor.has_value() ? tensor.value() : nullptr;
