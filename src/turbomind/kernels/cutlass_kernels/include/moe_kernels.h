...
#include "src/turbomind/kernels/cutlass_kernels/fp8_blockscale_gemm/fp8_blockscale_gemm.h"
...
struct MOEParallelismConfig
{
        ...
        , cluster_rank(cluster_rank)
    {
        ...
        ft::FT_CHECK(ep_rank >= 0);
        ft::FT_CHECK(ep_size >= 1);
        ft::FT_CHECK(cluster_rank < cluster_size);
        ft::FT_CHECK(cluster_rank >= 0);
        ft::FT_CHECK(cluster_size >= 1);
        ft::FT_CHECK(ep_size == 1 || cluster_size == 1);
    }
...
public:
    ...
    virtual std::vector<cutlass_extensions::CutlassGemmConfig> getDefaultTactics(int num_tokens) = 0;
    ...
    virtual void gemm1(void const* const input, void* const output, void* const intermediate_result,
        int64_t const* const expert_first_token_offset, int const* permute_experts, TmaWarpSpecializedGroupedGemmInput tma_ws_input_template,
...
public:
    ...
    std::vector<cutlass_extensions::CutlassGemmConfig> getDefaultTactics(int num_tokens) override;
        ...
        int* active_expert_global_ids, int start_expert) override
    {
        ...
        return Self::gemm1(moe_gemm_runner_, block_scale_gemm_runner, static_cast<T const*>(input),
            static_cast<T*>(output), intermediate_result, expert_first_token_offset, permute_experts, tma_ws_input_template,
        ...
        bool use_shared_stream=false, cudaEvent_t shared_expert_event=nullptr, cudaStream_t shared_expert_stream=nullptr) override
    {
        ...
        return Self::gemm2(moe_gemm_runner_, block_scale_gemm_runner, static_cast<T const*>(input), gemm_output,
            ...
            active_expert_global_ids, start_expert, use_shared_stream, shared_expert_event, shared_expert_stream);
    }
