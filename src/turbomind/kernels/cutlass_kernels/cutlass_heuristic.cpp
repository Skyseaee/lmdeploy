...
TileShape get_cta_shape_for_config(CutlassTileConfig tile_config)
{
    ...
    default: turbomind::FT_THROW("[get_grid_shape_for_config] Invalid config");
    }
}
...
std::vector<CutlassGemmConfig> get_candidate_configs_sm100(CutlassGemmConfig::CandidateConfigTypeParam const config)
{
    ...
    {
        turbomind::FT_THROW("Not Implemented: SM100 GEMM candidates have not been defined.");
    }
#endif
}
...
std::vector<CutlassGemmConfig> get_candidate_configs_sm120(CutlassGemmConfig::CandidateConfigTypeParam const config)
{
    ...
    if (config & CutlassGemmConfig::GROUPED_GEMM)
    {
        ...
        {
            turbomind::FT_THROW("Not Implemented: SM120 group GEMM only supports nvfp4.");
        }
    }
    else
    {
        ...
        {
            turbomind::FT_THROW("Not Implemented: SM120 GEMM only supports nvfp4.");
        }
    }
#endif
    ...
    int const split_k_limit, size_t const workspace_bytes, int const multi_processor_count, int const is_weight_only)
{
    ...
    {
        turbomind::FT_THROW("Heuristic failed to find a valid config.");
    }
