...
#define SHAPE_CASE(M, N, K)                                                                                            \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            throw std::runtime_error(turbomind::fmtstr(                                                                \
...
#undef SHAPE_CASE
    default: throw std::runtime_error(turbomind::fmtstr("Unsupported config %d for MoE gemm.", (int) gemm_config.cluster_shape));
    }
...
#define DEFAULT_CASE(SMVERSION)                                                                                        \
    case cutlass_extensions::CutlassTileConfigSM##SMVERSION::Undefined: throw std::runtime_error("GEMM config undefined."); break;   \
    case cutlass_extensions::CutlassTileConfigSM##SMVERSION::ChooseWithHeuristic:                                      \
        ...
        break;                                                                                                         \
    default: throw std::runtime_error(turbomind::fmtstr("Unsupported config %d for MoE gemm.", (int) gemm_config.tile_config_sm##SMVERSION)); break;
    ...
    if (gemm_config.sm_version == 90)
    {
        ...
        {
            throw std::runtime_error("Unsupported SM90 configuration requested");
        }
    }
    else if (gemm_config.sm_version >= 100 && gemm_config.sm_version < 120)
    {
        ...
        {
            throw std::runtime_error("Unsupported SM100 configuration requested");
        }
    }
    else if (gemm_config.sm_version == 120)
    {
        TM_LOG_TRACE("At %s, SM120 config=%d", __PRETTY_FUNCTION__, (int) gemm_config.tile_config_sm120);
