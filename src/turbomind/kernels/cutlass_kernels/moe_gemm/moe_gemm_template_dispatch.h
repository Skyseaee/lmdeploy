    ...
    static void call(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_)
    {
        ...
        if (!inputs.use_fused_moe)
        {
                              ...
                              cutlass_extensions::EpilogueOpDefault>)
            {
                ...
                {
                    FT_CHECK_WITH_INFO(inputs.scales == nullptr && inputs.biases == nullptr && inputs.alpha_scales,
...
static void dispatch(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_)
{
    ...
    else
    {
        throw std::runtime_error(
            turbomind::fmtstr("Cutlass gemm. Not instantiated for arch %d with stages set to %d", Arch::kMinComputeCapability, Stages));
    }
}
...
void dispatchGemmConfig(GroupedGemmInput<T, WeightType, GemmOutputType, GemmOutputType> inputs, int sm_count_)
{
    ...
    case 4:
        ...
        break;
    default: throw std::runtime_error(turbomind::fmtstr("dispatchGemmConfig does not support stages %d", inputs.gemm_config.stages)); break;
    }
}
...
MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::getTmaWarpSpecializedConfigs(int sm)
{
    ...
    FT_CHECK_WITH_INFO(!(enable_blackwell && enable_hopper), "Blackwell and hopper flags are mutually exclusive");
    ...
    GroupedGemmInput<T, WeightType, ScaleBiasType, OutputType> inputs, TmaWarpSpecializedGroupedGemmInput hopper_inputs)
{
    ...
    FT_CHECK_WITH_INFO(sm_ >= 90 || !inputs.gemm_config.is_tma_warp_specialized,
    ...
    else if (sm_ >= 90)
    {
            ...
            && !use_w4afp8)
        {
            ...
            {
                FT_CHECK_WITH_INFO(inputs.gemm_config.sm_version == sm_, turbomind::fmtstr("Using SM %d configuration for SM %d device",
                    inputs.gemm_config.sm_version, sm_));
                FT_CHECK_WITH_INFO(inputs.biases != nullptr || hopper_inputs.ptr_c == nullptr,
                ...
                auto select_function = [&]()
                {
                    ...
                    default: throw std::runtime_error(turbomind::fmtstr("Unimplemented fusion %d requested", (int) hopper_inputs.fusion));
                    };
                };
        ...
        if constexpr (use_w4afp8)
        {
            if (inputs.gemm_config.is_tma_warp_specialized)
            {
                ...
                {
                    throw std::runtime_error(turbomind::fmtstr("Invalid GEMM K size %d", (int) inputs.k));
                }
                return;
            };
        }
        ...
        {
            FT_CHECK_WITH_INFO(!use_fp8, "No fallback FP8 implementation available");
            FT_CHECK_WITH_INFO(use_w4afp8 || !hopper_inputs.isValid(),
                "Non-specialized Hopper implementation is being rerouted to fallback implementation so input "
                "information is not required");
            FT_CHECK_WITH_INFO(!inputs.gemm_config.is_tma_warp_specialized,
                "GEMM config is for SM90 configuration, but this configuration is not valid for Hppper");
            FT_CHECK_WITH_INFO(inputs.gemm_config.sm_version == 80,
                turbomind::fmtstr("Using SM %d configuration for SM80 fallback implementation", inputs.gemm_config.sm_version));
            if constexpr (use_fp8)
            {
                dispatchMoeGemmToCutlass<T, WeightType, ScaleBiasType, cutlass::arch::Sm89, EpilogueTag>(
                    inputs, multi_processor_count_);
...
#define CALC_SIZE_FUSION(FUSION)                                                                                       \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            TM_LOG_TRACE("Unsupported config skipped when calculating MOE workspace size %s", e.what());             \
        }                                                                                                              \
    } while (0)
