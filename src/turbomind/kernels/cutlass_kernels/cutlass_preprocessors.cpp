...
std::vector<int> get_permutation_map(QuantType quant_type)
{
    ...
    {
        throw std::runtime_error("Invalid quantization type for LDSM permutation");
    }
}
...
{
    TM_LOG_TRACE("%s start", __PRETTY_FUNCTION__);
    ...
    FT_CHECK_WITH_INFO(num_rows % B_ROWS_PER_MMA == 0,
        turbomind::fmtstr("Invalid shape for quantized tensor. Number of rows of quantized matrix must be a multiple of %d",
            B_ROWS_PER_MMA));
    FT_CHECK_WITH_INFO(num_cols % MMA_SHAPE_N == 0,
        turbomind::fmtstr("Invalid shape for quantized tensor. On turing/Ampere, the number of cols must be a multiple of %d.",
            MMA_SHAPE_N));

    FT_CHECK_WITH_INFO(size_t(B_ROWS_PER_MMA) == row_permutation.size(), "Unexpected number of LDSM rows permuted.");
    ...
    int8_t* transposed_quantized_tensor, int8_t const* quantized_tensor, std::vector<size_t> const& shape)
{
    ...
    FT_CHECK_WITH_INFO(!(col_bytes_trans % VECTOR_WIDTH) && !(col_bytes % VECTOR_WIDTH),
        turbomind::fmtstr("Number of bytes for rows and cols must be a multiple of %d. However, num_rows_bytes = %ld and "
               "num_col_bytes = %ld.",
    ...
    std::vector<size_t> const& shape, QuantType quant_type, LayoutDetails details)
{
    ...
    FT_CHECK_WITH_INFO(!(num_rows % elts_in_int32),
        turbomind::fmtstr("The number of rows must be a multiple of %d but the number of rows is %ld.", elts_in_int32, num_rows));

    uint32_t const* input_byte_ptr = reinterpret_cast<uint32_t const*>(quantized_tensor);
    uint32_t* output_byte_ptr = reinterpret_cast<uint32_t*>(interleaved_quantized_tensor);

    FT_CHECK_WITH_INFO(!(num_rows % rows_per_tile),
        turbomind::fmtstr("The number of rows must be a multiple of %d but the number of rows is %ld.", rows_per_tile, num_rows));
...
void print_vector(const std::vector<int8_t> data, int size, std::string info)
{
	...
	{
		printf("%d ", static_cast<int>(data[ii]));
	}
	printf("\n");
}
...
{
    int arch = turbomind::getSMVersion();
    ...
    //printf("shape: %d %d %d, quant bits: %d, num_bytes: %d\n", shape[0], shape[1], shape[2], get_weight_quant_bits(quant_type), (int)num_bytes);
    std::vector<int8_t> src_buf(num_bytes);
    std::vector<int8_t> dst_buf(num_bytes);

	
    //cudaDeviceSynchronize();
    //printf("** before copy input: "); turbomind::print_int4_to_screen(reinterpret_cast<const uint8_t*>(row_major_quantized_weight), 10);
    //cudaDeviceSynchronize();
    
    cudaMemcpy(src_buf.data(), row_major_quantized_weight, num_bytes, cudaMemcpyDefault);
    //std::copy(row_major_quantized_weight, row_major_quantized_weight + num_bytes, src_buf.begin());
    //cudaDeviceSynchronize();
    //print_vector(src_buf, 20, "finish copy input");

    // Works on row major data, so issue this permutation first.
    if (details.uses_imma_ldsm)
    {

	//printf("** 1.permute_B_rows_for_mixed_gemm\n");
	//print_vector(src_buf, 10, "step1. input");
        permute_B_rows_for_mixed_gemm(dst_buf.data(), src_buf.data(), shape, quant_type, arch);
        //print_vector(dst_buf, 10, "step1. output");
	src_buf.swap(dst_buf);
    }

    if (details.layoutB == LayoutDetails::Layout::COLUMN_MAJOR)
    {
	//printf("subbyte_transpose\n");
    ...
    {
	//printf("interleave_column_major_tensor\n");
    ...
    cudaMemcpy(preprocessed_quantized_weight, src_buf.data(), num_bytes, cudaMemcpyDefault);
