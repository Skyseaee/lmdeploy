    ...
    RunnerPtr const& runner, turbomind::DataType const& type, GemmDims const& dims, GemmIdType const& gemmId, bool hasWeightOnlyCudaKernel)
{
    ...
    turbomind::check_cuda_error(cudaStreamCreate(&mStream));
    ...
    turbomind::check_cuda_error(cudaStreamDestroy(mStream));
}
    ...
    int m, GemmIdType const& gemmId) const
{
    ...
    if (mSkip)
    {
        TM_LOG_DEBUG("Skip is set, no best config is set for this instance");
        return std::nullopt;
    }
...
{
    FT_CHECK_WITH_INFO(mTmpWorkspaceSizeInBytes > 0, "tmpWorkspaceSizeInBytes must be larger than 0");
    auto const status = cudaMalloc(&mWorkspaceTmp, mTmpWorkspaceSizeInBytes);
    FT_CHECK_WITH_INFO(status == cudaSuccess, "Can't allocate tmp workspace for GEMM tactics profiling.");
}
...
void GemmPluginProfiler<Config, RunnerPtr, GemmIdType, GemmIdHashType>::freeTmpData()
{
    ...
    FT_CHECK_WITH_INFO(status == cudaSuccess, "Can't free tmp workspace for GEMM tactics profiling.");
}
    ...
    int m, int n, int k, std::vector<Config> const& tactics)
{
    ...
    if (!foundOne)
    {
        ...
        TM_LOG_WARNING(msg.str().c_str());
        return std::nullopt;
    }
    std::cout << "m=" << m << ", n=" << n << ", k=" << k << ", bestTime=" << bestTime  << ", "<< bestConfig << std::endl;
    return {bestConfig};
}
    ...
    int m, int n, int k, Config const& tactic)
{
    ...
    float elapsed;
    turbomind::check_cuda_error(cudaEventElapsedTime(&elapsed, start, stop));
