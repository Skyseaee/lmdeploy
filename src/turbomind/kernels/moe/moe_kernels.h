...
#include "src/turbomind/kernels/lora/lora.h"
...
struct MOEParallelismConfig
{
        ...
        , ep_rank(ep_rank)
    {
        ...
        ft::FT_CHECK(tp_rank < tp_size);
        ft::FT_CHECK(tp_rank >= 0);
        ft::FT_CHECK(tp_size >= 1);
        ft::FT_CHECK(ep_rank < ep_size);
        ft::FT_CHECK(ep_rank >= 0);
        ft::FT_CHECK(ep_size >= 1);
    }
...
public:
    ...
    virtual size_t getWorkspaceSize(int64_t const num_rows, int64_t const hidden_size, int64_t const inter_size,
        ...
        = 0;
    virtual void setTactic(std::optional<tkc::CutlassGemmConfig> gemm1_config,
        std::optional<tkc::CutlassGemmConfig> gemm2_config)
        = 0;
    virtual std::vector<tkc::CutlassGemmConfig> getTactics() = 0;
    virtual std::vector<tkc::CutlassGemmConfig> getDefaultTactics(int num_tokens) = 0;
    ...
    virtual void runMoe(void const* input_activations, float const* gating_output, void const* fc1_expert_weights,
        ...
        bool use_shared_stream=false, cudaEvent_t shared_expert_event=nullptr, cudaStream_t shared_expert_stream=nullptr)
        = 0;
    ...
    virtual void gemm1(void const* const input, void* const output, void* const intermediate_result,
        int64_t const* const expert_first_token_offset, int const* permute_experts, HopperGroupedGemmInput hopper_input_template,
...
public:
    ...
    void setTactic(std::optional<tkc::CutlassGemmConfig> gemm1_config,
        std::optional<tkc::CutlassGemmConfig> gemm2_config) override
    {
        gemm1_config_ = std::move(gemm1_config);
        gemm2_config_ = std::move(gemm2_config);
    }

    std::vector<tkc::CutlassGemmConfig> getTactics() override
    {
        return moe_gemm_runner_.getConfigs();
    }

    std::vector<tkc::CutlassGemmConfig> getDefaultTactics(int num_tokens) override;

    static std::vector<tkc::CutlassGemmConfig> getTactics(int sm)
    {
    ...
    void runMoe(void const* input_activations, float const* gating_output, void const* fc1_expert_weights,
        ...
        MOEParallelismConfig parallelism_config, MOEExpertScaleNormalizationMode normalization_mode, bool use_lora,
        LoraParams& lora_params, cudaStream_t stream,
        bool use_shared_stream=false, cudaEvent_t shared_expert_event=nullptr, cudaStream_t shared_expert_stream=nullptr) override;
    ...
    static void gemm1(MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>& gemm_runner, T const* const input,
        T* const output, void* const intermediate_result, int64_t const* const expert_first_token_offset, int const* permute_experts,
    ...
    void gemm1(void const* const input, void* const output, void* const intermediate_result,
        ...
        int64_t const inter_size, int const num_experts_per_node, ActivationType fc1_activation_type,
        float const** alpha_scale_ptr_array, bool bias_is_broadcast, cudaStream_t stream,
        tkc::CutlassGemmConfig config) override
    {
        return Self::gemm1(moe_gemm_runner_, static_cast<T const*>(input), static_cast<T*>(output), intermediate_result,
            expert_first_token_offset, permute_experts, hopper_input_template, static_cast<WeightType const*>(fc1_expert_weights),
    ...
    void gemm2(void const* const input, void* const gemm_output, void* const final_output,
        ...
        tkc::CutlassGemmConfig config, bool use_shared_stream=false, cudaEvent_t shared_expert_event=nullptr, cudaStream_t shared_expert_stream=nullptr) override
    {
        return Self::gemm2(moe_gemm_runner_, static_cast<T const*>(input), gemm_output,
            ...
            alpha_scale_ptr_array, use_lora, fc2_lora, stream, parallelism_config, config, use_shared_stream, shared_expert_event, shared_expert_stream);
    }
...
private:
    ...
    std::optional<tkc::CutlassGemmConfig> gemm1_config_;
    std::optional<tkc::CutlassGemmConfig> gemm2_config_;
...
{
public:
    using Config = tkc::CutlassGemmConfig;
    ...
    void init(CutlassMoeFCRunnerInterface& runner, GemmToProfile gemm_to_profile, turbomind::DataType dtype,
       turbomind::DataType wtype, turbomind::DataType otype, int num_experts, int k, int64_t hidden_size,
    ...
    turbomind::DataType mDType{};
    turbomind::DataType mWType{};
    turbomind::DataType mOType{};
