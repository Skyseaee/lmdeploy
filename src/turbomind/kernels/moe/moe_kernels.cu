 ...
 */
 ...
 #include "src/turbomind/kernels/cutlass_extensions/include/cutlass_extensions/epilogue/thread/fused_activations.h"
 #include "src/turbomind/kernels/moe/moe_gemm/workspace.h"
 ...
 */
     ...
     int const start_expert, int const end_expert, MOEExpertScaleNormalizationMode norm_mode, cudaStream_t stream)
 {
     if (moeRoutingNeedsSoftmax(norm_mode))
     {
         ...
         default:
         {
             ...
             ft::FT_CHECK(softmax_temp_output != nullptr);
     ...
     int const end_expert, float epsilon, cudaStream_t stream)
 {
     ...
     //  recent one.
     FT_CHECK_WITH_INFO(k <= 2, "Current sparse mixer only supports k <= 2");
     ...
     int const end_expert, float mixer_epsilon, MOEExpertScaleNormalizationMode norm_mode, cudaStream_t stream)
 {
     ...
     {
         FT_CHECK_WITH_INFO(mixer_temp_output, "Sparse mixer output is null when running sparse mixer");
 ...
 __global__ void doActivationKernel(T* output, GemmOutputType const* gemm_result, float const* fp8_quant,
     ScaleBiasType const* bias_ptr, bool bias_is_broadcast, int64_t const* expert_first_token_offset, int const* permute_experts, int num_experts,
     int64_t inter_size, bool gated)
 {
     ...
     //float quant_scale_per_expert[16] = {3.737226,3.764706,1.718121,2.160337,5.12,6.4,1.21327,7.699248,3.084337,3.506849,13.744967,8.827586,4.452174,12.337349,2.235808,24.094118};
 ...
 void doActivation(T* output, GemmOutputType const* gemm_result, float const* fp8_quant, ScaleBiasType const* bias,
     bool bias_is_broadcast, int64_t const* expert_first_token_offset, int const* permute_experts, int num_experts, int64_t inter_size,
     int64_t num_tokens, ActivationType activation_type, cudaStream_t stream)
 {
     ...
     fn<<<blocks, threads, 0, stream>>>(output, gemm_result, fp8_quant, bias, bias_is_broadcast,
         expert_first_token_offset, permute_experts, num_experts, inter_size, isGatedActivation(activation_type));
 }
...
{
#ifdef ENABLE_BF16
        TM_LOG_ERROR("Lora cant support BF16 DataType now!");
#else
     int64_t const blocks = num_tokens;
     ...
     bool use_lora) const
 {
     ...
     TM_LOG_DEBUG("Mixture Of Experts Plugin requires workspace of %2f MiB", ws_size / 1024.f / 1024.f);
     return ws_size;
 }
 ...
 void CutlassMoeFCRunner<T, WeightType, OutputType, ScaleBiasType, Enable>::gemm1(
     ...
     bool bias_is_broadcast, cudaStream_t stream, tkc::CutlassGemmConfig config)
 {
     ...
     if (using_hopper_gemm1)
     {
         ...
         doActivation<T, UnfusedGemmOutputType>(output, static_cast<UnfusedGemmOutputType const*>(gemm_output),
             fc2_fp8_quant, fc1_expert_biases, bias_is_broadcast, expert_first_token_offset, permute_experts, num_experts_per_node,
 ...
 void CutlassMoeFCRunner<T, WeightType, OutputType, ScaleBiasType, Enable>::gemm2(
     ...
     cudaStream_t stream, MOEParallelismConfig parallelism_config, tkc::CutlassGemmConfig config,
     ...
     bool use_shared_stream, cudaEvent_t shared_expert_event, cudaStream_t shared_expert_stream)
 {
     ...
     FT_CHECK_WITH_INFO(finished == nullptr, "Using 'finished' is deprecated and will be removed in future versions");
     FT_CHECK_WITH_INFO(
         num_rows == active_rows, "Using 'finished' is deprecated and will be removed in future versions");
     ft::FT_CHECK(input_activations);
     ft::FT_CHECK(gating_output);
     ...
     ft::FT_CHECK(expert_for_source_row);
     ft::FT_CHECK(num_experts % parallelism_config.ep_size == 0);
     FT_CHECK_WITH_INFO(hidden_size >= 128 / cutlass::sizeof_bits<WeightType>::value,
         "Hidden size is too small to meet alignment requirements for MOE GEMM");
     FT_CHECK_WITH_INFO(hidden_size % (128 / cutlass::sizeof_bits<WeightType>::value) == 0,
         "Hidden size does not meet minimum alignment requirements for MOE GEMM");
     FT_CHECK_WITH_INFO(inter_size % (128 / cutlass::sizeof_bits<WeightType>::value) == 0,
     ...
     FT_CHECK_WITH_INFO(k * num_experts <= std::numeric_limits<int>::max(), "k * num_experts is too large");
     ...
     if (use_lora)
     {
         ...
         ft::check_cuda_error(cudaMemcpyAsync(host_permuted_rows.data(), permuted_rows_, expanded_num_rows * sizeof(int),
             cudaMemcpyDeviceToHost, stream));
         host_expert_first_token_offset.resize(num_experts_per_node + 1);
         ft::check_cuda_error(cudaMemcpyAsync(host_expert_first_token_offset.data(), expert_first_token_offset_,
     ...
     Self::gemm1(moe_gemm_runner_, permuted_data_, fc1_result_, glu_inter_result_, expert_first_token_offset_, permuted_experts_,
     ...
     Self::gemm2(moe_gemm_runner_, fc1_result_, fc2_result_, final_output, expert_first_token_offset_,
         ...
         stream, parallelism_config, *gemm2_config_, use_shared_stream, shared_expert_event, shared_expert_stream);
 ...
 template <class T, class WeightType, class OutputType, class ScaleBiasType, class Enable>
 std::vector<tkc::CutlassGemmConfig> CutlassMoeFCRunner<T, WeightType, OutputType, ScaleBiasType, Enable>::getDefaultTactics(int num_tokens)
 {
    tkc::CutlassGemmConfig gemm1_config, gemm2_config;
    gemm1_config.cluster_shape = tkc::ClusterShape::ClusterShape_2x1x1;
    gemm2_config.cluster_shape = tkc::ClusterShape::ClusterShape_2x1x1;
    gemm1_config.is_sm90 = true;
    gemm2_config.is_sm90 = true;

    if(num_tokens <= 32)
    {
        gemm1_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x16x128B;
        gemm2_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x16x128B;
    }
    else if(num_tokens <= 64)
    {
        gemm1_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x32x128B;
        gemm2_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x32x128B;
    }
    else if(num_tokens <= 128)
    {
        gemm1_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x64x128B;
        gemm2_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x64x128B;
    }
    else if(num_tokens <= 256)
    {
        gemm1_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape256x128x128B;
        gemm2_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x128x128B;

        gemm1_config.cluster_shape = tkc::ClusterShape::ClusterShape_1x1x1;
    }
    else if(num_tokens <= 1024)
    {
        gemm1_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape256x128x128B;
        gemm2_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape256x128x128B;

        gemm2_config.cluster_shape = tkc::ClusterShape::ClusterShape_1x1x1;
    }
    else // 1024+
    {
        gemm1_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape128x256x128B;
        gemm2_config.tile_config_sm90 = tkc::CutlassTileConfigSM90::CtaShape256x128x128B;

        gemm2_config.cluster_shape = tkc::ClusterShape::ClusterShape_1x2x1;
    }
 ...
 void makeLoadBalancedRoutingConfiguration(
     void* data_void, int num_experts, int num_tokens, int k, turbomind::DataType type, cudaStream_t stream)
 {
     FT_CHECK_WITH_INFO(type == turbomind::DataType::TYPE_FP32, "Routing configuration must be float");
     ft::check_cuda_error(
         cudaMemsetAsync(data_void, 0x0, int64_t{num_experts} * int64_t{num_tokens} * sizeof(float), stream));
 
     int stride = ceilDiv(num_experts, k);
 
     int blockDim = 256;
     int gridDim = ceilDiv(num_tokens, blockDim);
 ...
 std::vector<size_t> GemmProfilerBackend::getProfilerWorkspaces(int maxM, bool is_hopper)
 {
     ...
     size_t dtype_bytes = ft::Tensor::getTypeSize(mDType);
    //  float weight_bytes
    //      = mWType == turbomind::DataType::kINT4 ? 0.5f : static_cast<float>(tensorrt_llm::common::getDTypeSize(mWType));
     float weight_bytes = static_cast<float>( ft::Tensor::getTypeSize(mWType));
     size_t output_bytes = ft::Tensor::getTypeSize(mOType);
     size_t gemm_output_bytes = (mOType == turbomind::DataType::TYPE_FP8_E4M3)
     ...
     bool is_int_w_quant = mWType == turbomind::DataType::TYPE_INT8 /*|| mWType == turbomind::DataType::kINT4*/;
     bool is_fp8_w_quant = mWType == turbomind::DataType::TYPE_FP8_E4M3;
 ...
 std::function<void*()> GemmProfilerBackend::getWorkspacePointerGenerator(char* ws, int maxM, bool is_hopper)
 {
     ...
     {
         FT_CHECK_WITH_INFO(index < workspaces.size(), "Mismatching scratch space allocation");
     ...
     int original_num_tokens, Config const& tactic, char* workspace_ptr_char, cudaStream_t const& stream)
 {
     ...
     if (mWType == turbomind::DataType::TYPE_INT8 /*|| mWType == turbomind::DataType::kINT4*/)
     {
         ft::FT_CHECK(scale_1 && scale_2);
         quant_params = QuantParams::Int(scale_1, scale_2);
     }
     else if (mWType == turbomind::DataType::TYPE_FP8_E4M3)
     {
         ft::FT_CHECK(scale_1 && scale_2 && scale_3);
