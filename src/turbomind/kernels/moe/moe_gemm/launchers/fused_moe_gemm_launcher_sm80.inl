    ...
    int* kernel_occupancy)
{
    ...
    if (kernel_occupancy != nullptr)
    {
        ...
        if (smem_size > (48 << 10))
        {
            ...
            turbomind::check_cuda_error(cudaFuncGetAttributes(&attr, fused_moe::run_global<GemmType>));
        ...
        turbomind::check_cuda_error(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
    ...
    FT_CHECK_WITH_INFO(occupancy > 0, "GPU lacks the shared memory resources to run fused_moe kernel");
    ...
    if (GemmType::kSmemSize >= (48 << 10))
    {
        ...
        FT_CHECK_WITH_INFO(result == cudaSuccess,
    ...
    FT_CHECK_WITH_INFO(result == cudaSuccess, "Fail to execute fused moe kernel, cuda error " + std::to_string((int)(result)) );
}
} // namespace tensorrt_llm::kernels::cutlass_kernels
