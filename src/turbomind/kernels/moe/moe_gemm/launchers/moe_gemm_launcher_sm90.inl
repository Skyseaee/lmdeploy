    ...
    int const multi_processor_count, cudaStream_t stream, int* kernel_occupancy, size_t* workspace_size)
{
    ...
    if constexpr (!should_filter_sm90_gemm_problem_shape_v<TileShape, ClusterShape, T>)
    {
        ...
        turbomind::FT_CHECK(hopper_input.stride_a);
        turbomind::FT_CHECK(hopper_input.stride_b);
        turbomind::FT_CHECK(hopper_input.ptr_a);
        turbomind::FT_CHECK(hopper_input.ptr_b);
        ...
        FT_CHECK_WITH_INFO(calculated_ws_size <= hopper_input.gemm_workspace_size,
            "Workspace is size " +  std::to_string((int)(calculated_ws_size)) + " but only " + std::to_string((int)(hopper_input.gemm_workspace_size))
            + " were allocated");
