...
#include "src/turbomind/kernels/cutlass_extensions/include/cutlass_extensions/compute_occupancy.h"
#include "src/turbomind/kernels/cutlass_extensions/include/cutlass_extensions/epilogue_helpers.h"
#include "src/turbomind/kernels/cutlass_extensions/include/cutlass_extensions/gemm/kernel/default_fpA_intB_traits.h"
#include "src/turbomind/kernels/cutlass_extensions/include/cutlass_extensions/gemm/kernel/moe_cutlass_kernel.h"
#include "src/turbomind/kernels/cutlass_extensions/include/cutlass_extensions/gemm/threadblock/default_mma.h"
...
#include "src/turbomind/kernels/moe/moe_gemm/launchers/moe_gemm_launcher_sm90.h"
#include "src/turbomind/kernels/moe/moe_gemm/moe_gemm_kernels.h"
#include "src/turbomind/kernels/moe/moe_gemm/moe_sm90_traits.h"
#include "src/turbomind/kernels/moe/moe_gemm/launchers/fused_moe_gemm_launcher_sm80.h"
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* kernel_occupancy = nullptr)
{
    ...
    if (!use_fused_moe)
    {
        ...
        {
            FT_CHECK_WITH_INFO(weight_scales == nullptr && biases == nullptr && alpha_scale_ptr_array,
        ...
        FT_CHECK_WITH_INFO(occupancy > 0, "GPU lacks the shared memory resources to run GroupedGEMM kernel");
    ...
    int* occupancy = nullptr)
{
    ...
    {
        std::string errMsg = "Cutlass gemm. Not instantiated for arch" + std::to_string(Arch::kMinComputeCapability) 
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    case 4:
        ...
        break;
    default:
        std::string errMsg = "dispatchGemmConfig does not support stages " + std::to_string(gemm_config.stages);
        throw std::runtime_error(errMsg);
        break;
    }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    //         use_fused_moe, alpha_scale_ptr_array, stream, occupancy);
    //     break;
    // case cutlass_extensions::CutlassTileConfig::Undefined: throw std::runtime_error("GEMM config undefined."); break;
    ...
    //     break;
    // default: throw std::runtime_error("Config is invalid for same type tensorop GEMM."); break;
    // }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    //     throw std::runtime_error("GEMM config should have already been set by heuristic.");
    //     break;
    // default: throw std::runtime_error("Config is invalid for mixed type tensorop GEMM."); break;
    // }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy = nullptr)
{
    ...
    //     throw std::runtime_error("GEMM config should have already been set by heuristic.");
    //     break;
    // default: throw std::runtime_error("Unsupported config for float MoE gemm."); break;
    // }
}
    ...
    int sm)
{
    ...
    printf("[in MoeGemmRunner getConfigs]: find candidate_configs size: %d, ampere_configs size: %d\n", candidate_configs.size(), ampere_configs.size());
...
{
    int device{-1};
    turbomind::check_cuda_error(cudaGetDevice(&device));
    sm_ = turbomind::getSMVersion();
    ...
    bool use_fused_moe, float const** alpha_scale_ptr_array, cudaStream_t stream, int* occupancy)
{
    ...
    if (sm_ >= 70 && sm_ < 75)
    {
        throw std::runtime_error("The ffn-moe kernel for sm70 has not been compiled now!");
    ...
    {
        throw std::runtime_error("The ffn-moe kernel for sm75 has not been compiled now!");
    ...
    {
        throw std::runtime_error("The ffn-moe kernel for sm80/sm89 has not been compiled now!");
...
//             FT_CHECK_WITH_INFO(sm_ == 89, "For sm >= 80 and < 90, fp8 is only supported with sm == 89");
...
//         }
    ...
    else if (sm_ >= 90)
    {
        if constexpr (kernels::cutlass_kernels::isValidHopperMOESpecialisation<T, WeightType, EpilogueTag>())
        {
            ...
            {
                FT_CHECK_WITH_INFO(biases != nullptr || hopper_input.ptr_c == nullptr,
                    "Input biases and hopper input disagree if bias is enabled");
                FT_CHECK_WITH_INFO(hopper_input.isValid(), "Calling SM90 configuration with invalid hopper config");
                ...
                auto select_function = [&]()
                {
                    ...
                    default:
                        std::string errMsg = "Unimplemented fusion  " + std::to_string(int(hopper_input.fusion)) + " requested";
                        throw std::runtime_error(errMsg);
                    };
                };
        ...
        {
            FT_CHECK_WITH_INFO(!hopper_input.isValid(),
                "Non-specialised Hopper implementation is being rerouted to fallback implementation so input "
                "information is not required");
            FT_CHECK_WITH_INFO(!gemm_config.is_sm90,
                "GEMM config is for SM90 configuration, but this configuration is not valid for Hppper");

            throw std::runtime_error("sm90 ffn-moe kernel is failed! The ffn-moe kernel for sm80/sm89 has not been compiled now!");
            // dispatchMoeGemmToCutlass<T, WeightType, ScaleBiasType, cutlass::arch::Sm80, EpilogueTag>(A, B,
            //     weight_scales, biases, bias_is_broadcast, C, total_tokens_including_expert, total_rows, gemm_n, gemm_k,
            //     num_experts, gemm_config, multi_processor_count_, use_fused_moe, alpha_scale_ptr_array, stream,
            //     occupancy);
        ...
        {
            throw std::runtime_error("Configuration expects SM80 but configuration is not supported by SM80 kernels");
        }
    }
    else
    {
        throw std::runtime_error("Arch unsupported for MoE GEMM");
    }
}
...
size_t MoeGemmRunner<T, WeightType, OutputType, ScaleBiasType>::getMaxWorkspaceSize(int num_experts) const
{
    ...
    {
        TM_LOG_TRACE("Calling getMaxWorkspaceSize() with a new expert count %d vs %d", num_experts, num_experts_);
...
#define CALC_SIZE_FUSION(FUSION)                                                                                       \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            TM_LOG_TRACE("Unsupported config skipped when calculating MOE workspace size");                          \
        }                                                                                                              \
    } while (0)
        ...
        FT_CHECK_WITH_INFO(has_config, "Could not find valid config when calculating workspace size");
        return max_size;
    }
    else
    {
        throw std::runtime_error("Attempting to calculate Hopper GEMM workspace size with unsupported weight combination");
        return 0;
    }
}
    ...
    float const** alpha_scale_ptr_array, cudaStream_t stream, cutlass_extensions::CutlassGemmConfig chosen_conf)
{
    ...
    case ActivationType::Geglu:
        runGemm<cutlass_extensions::EpilogueOpDefaultFtGelu>(A, B, weight_scales, biases, bias_is_broadcast, C,
            ...
            alpha_scale_ptr_array, stream, chosen_conf);
        break;
    case ActivationType::InvalidType: throw std::runtime_error("Activation type for fpA_intB must be valid."); break;
    default: throw std::runtime_error("Invalid activation type."); break;
    }
}
