...
#include "src/turbomind/utils/quant_type_utils.cuh"
#include <float.h>
...
__global__ static void quantizedKernel(char4* dst, float4 const* src, int64_t const sizeDiv4, float const* scalePtr)
{
    for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < sizeDiv4; idx += blockDim.x * gridDim.x)
    {
        ...
        tmp.x = ft::cuda_cast<int8_t>(floatTmp.x * scale);
        tmp.y = ft::cuda_cast<int8_t>(floatTmp.y * scale);
        tmp.z = ft::cuda_cast<int8_t>(floatTmp.z * scale);
        tmp.w = ft::cuda_cast<int8_t>(floatTmp.w * scale);
        dst[idx] = tmp;
    }
}
...
__global__ static void quantizedKernel(char4* dst, half2 const* src, int64_t const sizeDiv4, float const* scalePtr)
{
    for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < sizeDiv4; idx += blockDim.x * gridDim.x)
    {
        ...
        tmp.x = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(half2Tmp.x) * scale);
        tmp.y = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(half2Tmp.y) * scale);
        tmp.z = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(half2Tmp2.x) * scale);
        tmp.w = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(half2Tmp2.y) * scale);
        dst[idx] = tmp;
    }
}
    ...
    char4* dst, __nv_bfloat162 const* src, int64_t const sizeDiv4, float const* scalePtr)
{
    for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < sizeDiv4; idx += blockDim.x * gridDim.x)
    {
        ...
        tmp.x = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(bfloat162Tmp.x) * scale);
        tmp.y = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(bfloat162Tmp.y) * scale);
        tmp.z = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(bfloat162Tmp2.x) * scale);
        tmp.w = ft::cuda_cast<int8_t>(ft::cuda_cast<float>(bfloat162Tmp2.y) * scale);
...
#pragma unroll
    for (int i = 0; i < NUM_ELTS; ++i)
    {
        ...
        localMax = ft::cuda_max(localMax, cuda_abs(val));
    }
}
...
#pragma unroll
    for (int i = 0; i < NUM_ELTS; ++i)
    {
        ...
        float2 val2 = ft::cuda_cast<float2>(val);
        val2.x *= scaleOrigQuant;
        val2.y *= scaleOrigQuant;
        QuantT quantVal = ft::cuda_cast<QuantT>(val2);
    ...
    float const* clampPtr, float* scalePtr, float* sumPtr, bool hasFp8MinScaling)
{
    ...
    T const clampMin = ft::cuda_cast<T>(clampPtr ? clampPtr[0] : -FLT_MAX);
    T const clampMax = ft::cuda_cast<T>(clampPtr ? clampPtr[1] : FLT_MAX);

    // Pack two elements in order to use higher through instructions.
    using T2 = typename ft::packed_as<T, 2>::type;
    using QuantT2 = typename ft::packed_as<QuantT, 2>::type;
    T2 const clampMin2 = ft::cuda_cast<T2, T>(clampMin);
    T2 const clampMax2 = ft::cuda_cast<T2, T>(clampMax);

    // The quantized data type's maximum value (upper-bound).
    static constexpr float MAX_QUANT_VAL = tensorrt_llm::common::QuantTypeStaticVals<QuantT>::MAX_VAL;
    // The minimum scaling factor (lower-bound).
    static constexpr float MIN_SCALING_FACTOR = tensorrt_llm::common::QuantTypeStaticVals<QuantT>::MIN_SCALING_FACTOR;
    static constexpr float MIN_SCALING_FACTOR_RCP =
        tensorrt_llm::common::QuantTypeStaticVals<QuantT>::MIN_SCALING_FACTOR_RCP;
    ...
    T2 localMax2 = ft::cuda_cast<T2, T>(T(1e-6f));
...
#pragma unroll
        for (int j = 0; j < NUM_ELTS2_PER_VEC; ++j)
        {
            ...
            localMax2 = ft::cuda_max(localMax2, cuda_abs(val2));
            // TODO: template the version that requires sum to avoid dynamic branching.
            if (sumPtr != nullptr)
            {
                localSum2.x += ft::cuda_cast<float>(val2.x);
                localSum2.y += ft::cuda_cast<float>(val2.y);
            }
        }
    ...
    float const rowMax = blockAllReduceMax(ft::cuda_cast<float>(ft::cuda_max<T, T2>(localMax2)));
    if (threadIdx.x == 0)
    {
        scalePtr[blockIdx.x]
            = hasFp8MinScaling ? ft::cuda_max(rowMax / MAX_QUANT_VAL, MIN_SCALING_FACTOR) : (rowMax / MAX_QUANT_VAL);
    }

    if (sumPtr != nullptr)
    {
        float rowSum[1] = {ft::cuda_sum<float>(localSum2)};
        ft::blockReduceSumV2<float, 1>(rowSum);
        if (threadIdx.x == 0)
        {
...
struct PackedVec
{
    typename ft::TypeConverter<Type>::Type elts[4];
...
#pragma unroll
    ...
    {
        localMax = ft::cuda_max(localMax, cuda_abs(vec.elts[i]));
    }
    ...
    localMax = ft::cuda_max(__shfl_xor_sync(uint32_t(-1), localMax, 1), localMax);
    if constexpr (CVT_NUM_THREADS_PER_SF == 4)
    {
        localMax = ft::cuda_max(__shfl_xor_sync(uint32_t(-1), localMax, 2), localMax);
    }
    // Get the final absolute maximum values.
    float vecMax = float(ft::cuda_max(localMax.x, localMax.y));
