...
inline void cutlass_gemm_caller(T*       res,
                         int                  batchCount,
                         int                  m,
                         int                  n,
                         int                  k,
  ...
  int64_t lda = k; //a.stride(0);
  int64_t ldb = k; //b.stride(1);
  int64_t ldc = n; //out.stride(0);

  using StrideC = Stride<int64_t, Int<1>, Int<0>>;
  StrideC c_stride{ldc, Int<1>{}, Int<0>{}};

  ElementAB const* a_ptr = reinterpret_cast<ElementAB const*>(input/*a.data_ptr()*/);
  ElementAB const* b_ptr = reinterpret_cast<ElementAB const*>(kernel/*b.data_ptr()*/);
  ElementD* c_ptr = reinterpret_cast<ElementD*>(res/*out.data_ptr()*/);

  typename Gemm::D::Arguments d_args{c_ptr, c_stride};

  using Epilogue = typename Gemm::Epilogue;
  auto evt_args =
      Epilogue::prepare_args(input_scale, kernel_scale); //(std::forward<EpilogueArgs>(epilogue_params)...);
  ...
  CUTLASS_CHECK(gemm_op(args, workspace.get(), stream));
}

template <typename Gemm, typename FallbackGemm, typename T, typename... EpilogueArgs>
inline void fallback_cutlass_gemm_caller(T*       res,
                         int                  batchCount,
                         int                  m,
                         int                  n,
                         int                  k,
  ...
  if (gemm_shared_mem_size <= max_shared_mem_per_block_opt_in) {
    return cutlass_gemm_caller<Gemm>(res, batchCount, m, n, k, stride_a, stride_b, stride_d, alpha, beta, input, kernel, input_scale, kernel_scale, stream,
                                     std::forward<EpilogueArgs>(args)...);
  } else {
    turbomind::FT_CHECK(fallback_gemm_shared_mem_size <= max_shared_mem_per_block_opt_in);
