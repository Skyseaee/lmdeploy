...
struct sm89_fp8_config_default {
  ...
  static void dispatch(OutType*       res,
                                    int                  batchCount,
                                    int                  m,
                                    int                  n,
                                    int                  k,
                                    ...
                                    cudaStream_t         stream, EpilogueArgs&&... args) {
    static_assert(std::is_same<InType, cutlass::float_e4m3_t>());
    ...
    if (np2 <= 4096) {
      ...
      return fallback_cutlass_gemm_caller<
          ...
          FallbackGemm>(res, batchCount, m, n, k, stride_a, stride_b, stride_d, alpha, beta, input, kernel, input_scale, kernel_scale, stream, std::forward<EpilogueArgs>(args)...);
    } else if (np2 <= 8192) {
...
template <typename InType, typename OutType,
          template <typename, typename> typename Epilogue,
          typename... EpilogueArgs>
inline void cutlass_gemm_sm89_fp8_dispatch(OutType*       res,
                                    int                  batchCount,
                                    int                  m,
                                    int                  n,
                                    int                  k,
