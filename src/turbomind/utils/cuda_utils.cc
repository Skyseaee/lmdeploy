...
#ifdef ENABLE_FP8
template void
print_to_file(const __nv_fp8_e4m3*, const int, const char*, cudaStream_t, std::ios::openmode);
#endif
...
void check_max_val(const T* result, const int size)
{
    ...
    for (int i = 0; i < size; i++) {
        float val = fabs(static_cast<float>(tmp[i]));
        if (val > max_val) {
            max_val = val;
        }
    }
...
#endif
#ifdef ENABLE_FP8
template void check_max_val(const __nv_fp8_e4m3* result, const int size);
#endif
