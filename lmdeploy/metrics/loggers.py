...
class PrometheusStatLogger(StatLoggerBase):
    ...
    def __init__(self, model_name: str, max_model_len: int, dp_rank: int = 0):
        ...
        self.gauge_scheduler_decode_count = prometheus_client.Gauge(
            name='lmdeploy:num_requests_decode_count',
            documentation='Number of requests in decode phase.',
            labelnames=labelnames).labels(*labelvalues)
        
        self.gauge_scheduler_prefill_count = prometheus_client.Gauge(
            name='lmdeploy:num_requests_prefill_count',
            documentation='Number of requests in prefill phase.',
            labelnames=labelnames).labels(*labelvalues)
        ...
        self.gauge_hit_rate = prometheus_client.Gauge(
            name='lmdeploy:gpu_prefix_cache_hit_rate',
            documentation='Hit rate of kv-cache.',
            labelnames=labelnames).labels(*labelvalues)
        ...
        self.counter_gpu_prefix_cache_hit = prometheus_client.Counter(
            name='lmdeploy:cache_query_hit',
            documentation='Number of prefix cache hit.',
            labelnames=labelnames).labels(*labelvalues)
        
        self.counter_gpu_prefix_cache_total = prometheus_client.Counter(
            name='lmdeploy:cache_query_total',
            documentation='Number of prefix cache total.',
            labelnames=labelnames).labels(*labelvalues)
        ...
        self.histogram_iter_token_latency = \
            prometheus_client.Histogram(
                name='lmdeploy:iteration_token_latency',
                documentation='Histogram of latency between two tokens.',
                buckets=[
    ...
    def record_schedule(self, stats: SchedulerStats) -> None:
        """Report scheduler-related metrics to prometheus."""
        ...
        self.gauge_hit_rate.set(stats.hit_rate)
        self.counter_gpu_prefix_cache_hit.inc(stats.cache_query_hit)
        self.counter_gpu_prefix_cache_total.inc(stats.cache_query_total)
        # self.gauge_scheduler_decode_count.set(stats.decode_count)
        # self.gauge_scheduler_prefill_count.set(stats.prefill_count)
    ...
    def record_iteration(self, stats: IterationStats) -> None:
        ...
        self.histogram_iteration_tokens.observe(stats.new_generation_tokens)

        if stats.time_to_first_tokens is not None:
            self.histogram_time_to_first_token.observe(stats.time_to_first_tokens)
        if stats.time_per_output_tokens is not None:
            self.histogram_time_per_output_token.observe(stats.time_per_output_tokens)
        if stats.itl is not None:
            self.histogram_iter_token_latency.observe(stats.itl)
    
    def record_finish(self, stats: FinishedRequestStats) -> None:
        """Report finished request-related metrics to prometheus."""
