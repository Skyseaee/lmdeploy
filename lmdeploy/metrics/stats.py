...
class SchedulerStats:
    ...
    # decode / prefill reqs in Turbomind engine
    ...
    def update_from_schedule_metrics(self, scheduled_metrics: ScheduleMetrics):
        ...
        self.num_total_reqs = scheduled_metrics.total_seqs
        self.gpu_cache_usage = 1.0 - (scheduled_metrics.free_blocks / scheduled_metrics.total_blocks)
        self.num_decode_reqs = scheduled_metrics.decode_count
        self.num_prefill_reqs = scheduled_metrics.prefill_count
        self.hit_rate = scheduled_metrics.hit_rate
        self.cache_query_hit = scheduled_metrics.cache_query_hit
        self.cache_query_total = scheduled_metrics.cache_query_total
    ...
    def __repr__(self):
        ...
        return ('SchedulerStats(\n'
                ...
                f'  num_decode_reqs={self.num_decode_reqs},\n'
                f'  num_prefill_reqs={self.num_prefill_reqs},\n'
                f'  cache_query_hit={self.cache_query_hit},\n'
                f'  cache_query_total={self.cache_query_total},\n'
                ')')
...
class RequestState:
    ...
    def finish_stats(self) -> 'FinishedRequestStats':
        """
        Return the stats of a finished request.
    ...
    def __repr__(self):
        ...
        return ('RequestState(\n'
                ...
                f'  lastest_token_time={self.lastest_token_time:.6f},\n'
                f'  finish_time={self.finish_time:.6f},\n'
...
class IterationStats:
    """Stats associated with one token generation iteration."""

    def __init__(self):
        self.iteration_timestamp = time.time()
        self.new_generation_tokens = 0
        self.prompt_tokens = 0
        self.time_to_first_tokens: Optional[float] = None
        self.time_per_output_tokens: Optional[float] = None
    ...
    def __repr__(self):
        ...
        return ('IterationStats(\n'
                ...
                f'  prompt_tokens={self.prompt_tokens},\n'
                f'  time_to_first_tokens={self.time_to_first_tokens},\n'
                f'  time_per_output_tokens={self.time_per_output_tokens},\n'
                f'  itl={self.itl},\n'
                ')')
    ...
    def update_from_output(self, outputs: EngineOutput, req_state: RequestState):
        ...
        if req_state.first_token_time == 0.0:
            req_state.first_token_time = outputs.req_metrics.token_timestamp
            self.prompt_tokens = req_state.prompt_tokens
            self.time_to_first_tokens = self._time_since(req_state.arrival_time)
        else:
            self.itl = self._time_since(req_state.lastest_token_time)
            self.time_per_output_tokens = self._time_since(req_state.lastest_token_time) / self.new_generation_tokens
