...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        # print(f"cu_window_seqlens v0: {cu_window_seqlens.shape}, {cu_window_seqlens.dtype}, {cu_window_seqlens}")
        ...
        if self.config._attn_implementation == 'sdpa':
            raw_attention_mask = torch.zeros([1, seq_len, seq_len], device=hidden_states.device, dtype=torch.bool)
            for i in range(1, len(cu_seqlens)):
                raw_attention_mask[..., cu_seqlens[i - 1] : cu_seqlens[i], cu_seqlens[i - 1] : cu_seqlens[i]] = True
            ...
            ##  for eager
            raw_attention_mask = torch.full(
                    [1, seq_len, seq_len], torch.finfo(torch.float32).min, device=hidden_states.device, dtype=torch.float32
                )
            for i in range(1, len(cu_seqlens)):
                raw_attention_mask[..., cu_seqlens[i - 1] : cu_seqlens[i], cu_seqlens[i - 1] : cu_seqlens[i]] = 0
        ...
        if self.config._attn_implementation in ["eager", "sdpa"]:
            ...
            # print(f"🌟spatial_merge_unit: {self.spatial_merge_unit}")
            # print(f"🌟hidden_state: {hidden_states.shape}, {hidden_states.dtype}")
            ...
            # print(f"🌟cu_window_seqlens: {cu_window_seqlens.shape}, {cu_window_seqlens.dtype}, {cu_window_seqlens}")
            # print(f"🌟raw_attention_mask: {raw_attention_mask.shape}, {raw_attention_mask.dtype}, {raw_attention_mask[0,:,:]}")
            # print(f"🌟window_attention_mask: {window_attention_mask.shape}, {window_attention_mask.dtype}, {window_attention_mask[0,:,:]}")
