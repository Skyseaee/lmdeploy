...
class MiniCPMVImageProcessor(BaseImageProcessor):
    ...
    def __init__(
        ...
        self.resize_sampling = kwargs.pop("resize_sampling", Image.Resampling.BILINEAR)
    ...
    def slice_image(
        ...
        if best_grid is None:
            ...
            #print(f"best_resize size: {best_size}")
            source_image = image.resize(best_size, resample=self.resize_sampling)
        else:
            # source image, down-sampling and ensure divided by patch_size
            best_resize = self.find_best_resize(original_size, scale_resolution, patch_size)
            source_image = image.copy().resize(best_resize, resample=self.resize_sampling)
            #print(f"best_resize size: {best_resize}")
            refine_size = self.get_refine_size(
                original_size, best_grid, scale_resolution, patch_size, allow_upscale=True
            )
            #print(f"best_resize= {best_resize}, refine_size={refine_size}")
            refine_image = image.resize(refine_size, resample=self.resize_sampling)
