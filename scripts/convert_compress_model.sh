#!/usr/bin/env bash
################################################################################
# @Copyright: 2023 Shopee. All Rights Reserved.
  ...
  if [ $method = calibrate ] || [ $method = smooth_quant ] || \
     ...
     [ $method = finegrained_fp8 ]; then
       ...
       if [ $method = calibrate ] || [ $method = smooth_quant ]; then
        ...
        # for CompassMax, we set batch_size bigger to avoid unactivated experts
        if [ "$architecture" = CompassMoeForCausalLM ] && [ "$num_experts" = 16 ]; then
          extra_args+="--batch-size 16 "
        fi
...
function modelopt_quantize_model() {
  ...
  if [ $precision = awq-w4a16 ] || [ $precision = sq-int8 ] || \
     [ $precision = fp8 ] || [ $precision = fp4 ]; then
    if [ $precision = awq-w4a16 ]; then
      extra_args+="--qformat int4_awq "

    elif [ $precision = awq-w4a8 ]; then
...
#      extra_args+="--kv_cache_qformat fp8 "
      export_fmt="onellm"

    elif [ $precision = fp4]; then
      extra_args+="--qformat nvfp4 "
#      extra_args+="--kv_cache_qformat fp8 "
    fi
    python3 ${SCRIPT_DIR}/modelopt_quantize.py \
      ...
      --calib_size 512 --batch_size 16 \
      --dataset ultrachat_2k \
      ${extra_args}
  else
    echo "Precision only supports awq-w4a16, awq-w4a8, sq-int8 or fp8"
    exit 1
  fi
}
...
# add model_format to config.yaml
python3 ${SCRIPT_DIR}/fill_config.py \
  --model_path ${MODEL_PATH} \
  --model_format ${precision} \
  --output_model_path ${quant_model_path}
