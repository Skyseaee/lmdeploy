...
std::vector<std::shared_ptr<std::unordered_map<std::string, triton::Tensor>>>
prepareRequest(std::string          config_file,
               const int            node_id,
               const int            gpu_count,
               std::vector<void*>*  pointer_record,
               ...
               ft::GenerationConfig generation_config)
{
    ...
    const auto        request_hyperparameter = reader["request"];
    const size_t request_batch_size = request_hyperparameter["request_batch_size"].as<int>();
    std::cerr << "request_batch_size=" << request_batch_size << "\n";

    const int start_id      = request_hyperparameter["start_id"].as<int>();
    const int end_id        = request_hyperparameter["end_id"].as<int>();
    const int max_input_len = request_hyperparameter["max_input_len"].as<int>();
    ...
    RequestParam param;
    param.beam_width                 = request_hyperparameter["beam_width"].as<int>();
    param.request_output_len         = request_hyperparameter["request_output_len"].as<int>();
    param.beam_search_diversity_rate = request_hyperparameter["beam_search_diversity_rate"].as<float>();
    param.runtime_top_k              = request_hyperparameter["top_k"].as<uint>();
    param.runtime_top_p              = request_hyperparameter["top_p"].as<float>();
    param.temperature                = request_hyperparameter["temperature"].as<float>();
    param.len_penalty                = request_hyperparameter["len_penalty"].as<float>();
    param.repetition_penalty         = request_hyperparameter["repetition_penalty"].as<float>();
    param.presence_penalty           = request_hyperparameter["presence_penalty"].as<float>();
    param.min_length                 = request_hyperparameter["min_length"].as<int>();
...
int threadCreateModelInstances(std::shared_ptr<LlamaTritonModel>           model,
                               std::vector<std::unique_ptr<ModelRequest>>* model_instances,
                               const int                                   device_id,
                               const int                                   rank)
{
    printf("[INFO] rank = %d \n", rank);
    ...
    model->processWeights(device_id, rank);
    model->createEngine(device_id, rank);

    auto model_instance            = model->createModelInstance(device_id);
...
int threadForward(std::unique_ptr<ModelRequest>* model_instance,
                  ft::TensorMap&                 request,
                  ...
                  bool                           stream_output = false;)
{
    ...
    ft::ModelRequest::InputParam input_param{request, generation_config, session_param, stream_output};

    ft::ModelRequest::OutputParam output_param = (*model_instance)->forward(request, comm);

    output_tensors = output_param.tensors;
    cudaDeviceSynchronize();
    return 0;
}
...
public:
    ScopedGIL()
    {
        std::cout << "[ScopedGIL] GIL Acquired (mock)\n";
    }
    ~ScopedGIL()
    {
        std::cout << "[ScopedGIL] GIL Released (mock)\n";
    }
};
...
*/
    ...
    // NOTE(Alan): 可以通过CUDA_VISIBLE_DEVICES来控制使用的GPU数量
    ...
    std::string config_file =
        argc >= 2 ? std::string(argv[1]) : "/home/workcode/examples/configs/compass_smoe_fp8.yaml";
    ...
    const std::string data_type_str       = ft_instance_hyperparameter["data_type"].as<std::string>();
    ...
    bool              enable_expert_para  = ft_instance_hyperparameter["enable_expert_para"].as<bool>();

    YAML::Node instance_reader;
    try {
        if (!model_dir.empty()) {
            const std::string config_file{model_dir + "/config.yaml"};
            instance_reader = YAML::LoadFile(config_file);
        }
    }
    catch (const YAML::Exception& e) {
        std::cerr << "Error reading YAML config: " << e.what() << std::endl;
    ...
    std::string instance_config = YAML::Dump(instance_reader);

    turbomind::DataType data_type;

    if (data_type_str == "fp16") {
    ...
    auto hack_ffi_ctx_factory = [] { return std::static_pointer_cast<void>(std::make_shared<ScopedGIL>()); };

    // Step 1: Create Model
    auto model = std::make_shared<LlamaTritonModel>(data_type, model_dir, instance_config, hack_ffi_ctx_factory);
    std::count << "engine config: " << instance_reader << std::endl;
    std::cout << model->toString();

    // step 2: Create model instances
    std::vector<std::unique_ptr<ModelRequest>> model_instances((size_t)gpu_count);
    std::vector<std::thread>                   threads;
    for (int device_id = 0; device_id < gpu_count; device_id++) {
        const int rank = node_id * gpu_count + device_id;

        threads.push_back(std::thread(threadCreateModelInstances, model, &model_instances, device_id, rank));
    }
    for (auto& t : threads) {
        t.join();
    }
    ...
    std::string        csv_name = argc >= 3 ? std::string(argv[2]) : "/home/workcode/examples/start_ids.csv";
    std::vector<void*> pointer_record;  // Used to prevent the pointers are
                                        // release after leaving functions

    ft::GenerationConfig generation_config;
    ft::SessionParam     session_param{.id = 0, .step = 0, .start_flag = true, .end_flag = false};

    std::vector<std::shared_ptr<std::unordered_map<std::string, triton::Tensor>>> request_list =
        prepareRequest(ini_name, node_id, gpu_count, &pointer_record, csv_name, generation_config);
    ...
    if (node_id == 0) {
        ...
        if (!outFile.is_open()) {
            ...
            outFile << "========== split for model info ==========\n";

            std::cout << "sequence length: ";
            outFile << "sequence length: ";
            for (int i = 0; i < batch_size; ++i) {
                std::cout << (i ? ", " : "") << seq_lens[i];
                outFile << (i ? ", " : "") << seq_lens[i];
            }
            std::cout << "\n";
            outFile << "\n";
    ...
    bool enable_time_statistic = argc >= 5 ? (std::stoi(std::string(argv[4])) == 1) : false;

    if (enable_time_statistic) {
        // test time
